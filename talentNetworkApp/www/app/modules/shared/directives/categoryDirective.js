...
(function(app){
	...
	app.directive('categoryPost',categoryPost);

	categoryPost.$inject = ['loader', 'categoryService','responseCodeHandling', '$rootScope','$ionicModal','$ionicScrollDelegate','$timeout','$cordovaSocialSharing','$cordovaAppAvailability','$cordovaToast'];

	function categoryPost(loader, categoryService, responseCodeHandling, $rootScope,$ionicModal,$ionicScrollDelegate,$timeout,$cordovaAppAvailability,$cordovaToast){
		return {
			restrict: 'EA',
			templateUrl: './app/modules/shared/templates/categoryDirective.tpl.html',
			scope:{
				type: '=type',
				data: '=data'
			},
       ...
       $scope.checkZommedOrNot=function(){
        ...
        $scope.toggleAudio = function(){
            loader.show();
             ...
             $scope.media_url=$sce.trustAsResourceUrl("https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/"+$scope.id +"&amp;auto_play=true&amp;");
            ...
            //  	}
          	...
          	var error=function (err){
               ...
               singlePost.fan_count=singlePost.fan_status? parseInt(singlePost.fan_count)-1 :parseInt(singlePost.fan_count) +1;
            	...
            	categoryService.like_Unlike_Media(null,dataToSend,success,error);
        	}
          else{}
        }
		}
